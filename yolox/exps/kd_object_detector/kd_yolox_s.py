...
class Exp(MyExp):
    def __init__(self, dataset_name=None, classes=None, exp_name=None, mlflow=None, grayscale=False, epochs=500, require_dataset = False):
        ...
        logger.debug("Initializing custom student 'kd_yolox_s.py'.")
        ...
        # --------------- transform config ----------------- #
        self.mosaic_prob = 0.75                 # 1.0
        self.mixup_prob = 0.2                   # 1.0
        self.hsv_prob = 0.75                    # 1.0
        self.flip_prob = 0.5
        ...
        # --------------  training config --------------------- #
        self.warmup_epochs = 10                 # 5
        self.max_epoch = epochs                 # 300
        self.warmup_lr = 0                      # minimum learning rate during warmup
        self.min_lr_ratio = 0.05
        self.basic_lr_per_img = 0.01 / 12.0     # lower (0.02 or 0.05), higher (0.005 or 0.001), learning rate for one image. During training, lr will multiply batchsize.
        self.scheduler = "yoloxwarmcos"         # name of LRScheduler
        self.no_aug_epochs = 30                 # 15 last #epoch to close augmention like mosaic
        self.ema = True                         # apply EMA during training
        self.weight_decay = 5e-4                # weight decay of optimizer (5 × 10⁻⁴ = 0.0005)
