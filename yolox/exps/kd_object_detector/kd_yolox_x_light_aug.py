...
class Exp(MyExp):
    def __init__(self, dataset_name=None, classes=None, exp_name=None, mlflow=None, grayscale=False, epochs=500, require_dataset = False):
        ...
        logger.debug("Initializing teacher 'kd_yolox_x_light_aug.py'.")
        ...
        self.flip_prob = 0.5
        self.degrees = 5.0                      # rotation angle range, for example, if set to 2, the true range is (-2, 2)
        self.translate = 0.05                   # translate range, for example, if set to 0.1, the true range is (-0.1, 0.1)
        self.mosaic_scale = (0.9, 1.1)          # (0.1, 2)
        self.enable_mixup = True                # apply mixup aug or not
        self.mixup_scale = (0.9, 1.1)
        self.shear = 1.0                        # shear angle range, for example, if set to 2, the true range is (-2, 2)
        # self.scale = (0.1, 2)
