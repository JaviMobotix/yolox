...
def launch(
    ...
    logger.debug(f"Launch training.")

    # Compute size of GPU pool.
    world_size = num_machines * num_gpus_per_machine

    if world_size > 1:
        logger.debug(f"Launch training with {str(world_size)} GPUs.")
        ...
        logger.debug(f"Launch training with {str(world_size)} GPU.")
        main_func(*args)
