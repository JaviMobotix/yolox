    ...
    def cache_images(
        ...
        if self.cache_type == "ram":
            if mem_required > mem.available:
                logger.info(
                    f"There is more {mem_required / gb:.1f} GB RAM required, "
                    f"than {mem.available / gb:.1f}/{mem.total / gb:.1f} GB RAM available."
                )
                self.cache = False
            else:
                logger.info(
        ...
        if self.cache and self.imgs is None:
            if self.cache_type == 'ram':
                ...
                logger.info("Using (" + str(num_imgs) +") cached images in RAM to accelerate training!")
            else:   # 'disk'
