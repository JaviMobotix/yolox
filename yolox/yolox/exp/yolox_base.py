...
class Exp(BaseExp):
    def __init__(self, mlflow=None, grayscale=False):
        super().__init__()
        ...
        # Start evaluation epoch, first epochs are not evaluated #Added by Javi Mota
    ...
    def get_model(self):
        ...
        if getattr(self, "model", None) is None:
            ...
            backbone = YOLOPAFPN(self.depth, self.width, in_channels=in_channels, act=self.act, in_shape=self.dim)  
            head = YOLOXHead(self.num_classes, self.width, in_channels=in_channels, act=self.act, class_weights=self.class_weights)
    ...
    def preprocess(self, inputs, targets, tsize):
        ...
        inputs = torch.nan_to_num(inputs, nan=255.0, posinf=255.0, neginf=0.0)
        targets = torch.nan_to_num(targets, nan=255.0, posinf=255.0, neginf=0.0)
        inputs = torch.floor(inputs)
        targets = torch.floor(targets)
        return inputs, targets

    def get_optimizer_orig(self, batch_size):
